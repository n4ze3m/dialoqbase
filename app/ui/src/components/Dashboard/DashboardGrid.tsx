      ...
      {status === "success" && data.length > 0 && (
        <div className="grid grid-cols-1 mt-6 gap-4 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {data.map((bot: any) => (
            <Link
              to={`/bot/${bot.id}`}
              className="flex rounded-md hover:shadow-lg transition-shadow duration-300 ease-in-out cursor-pointer border border-gray-200 bg-white dark:bg-[#262626] dark:border-gray-700 dark:hover:bg-[#232323] dark:hover:border-[#232222] hover:bg-gray-50"
              key={bot.id}
            >
